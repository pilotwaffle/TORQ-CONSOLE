...
def register_prince_flowers_agent():
    ...
    print(f"[LIST] Description: {agent.description}")
    print(f"[OK] Status: {'Available' if agent.available else 'Unavailable'}")
...
class PrinceFlowersCLI:
    ...
    async def initialize(self):
        ...
        print("[BOT] Prince Flowers Integration CLI v0.70.0")
        print("=" * 50)
    ...
    def show_help(self):
        """Show CLI help."""
        help_text = """
[BOT] Prince Flowers Integration CLI Commands:
...
The agent supports web search, research, analysis, and complex reasoning tasks.
    ...
    async def process_query(self, query: str, test_mode: bool = False):
        ...
        try:
            ...
            print(f"\n[OK] Response ({response.execution_time:.2f}s, {response.confidence:.1%} confidence):")
            print("-" * 50)
            ...
            if response.metadata:
                print(f"\n[LIST] Metadata: {json.dumps(response.metadata, indent=2, default=str)}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "demo":
            ...
            print("[START] Starting Prince Flowers Integration CLI...")
            try:
                asyncio.run(main())
            except KeyboardInterrupt:
                print("\n👋 Goodbye!")
