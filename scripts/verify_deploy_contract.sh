#!/usr/bin/env bash
# scripts/verify_deploy_contract.sh
...
# Read a top-level JSON key from stdin.
pykey() {
...
v = d.get(key, "")
# Normalize None -> empty string (contract says never null, but guard anyway)
print("" if v is None else v)
PY
}

r1key() { printf '%s' "$RESP1" | pykey "$1"; }
r2key() { printf '%s' "$RESP2" | pykey "$1"; }
...
VER="$(r1key version)"
if [[ "$APP_V" != "$VER" ]]; then
  echo "FAIL [version==app_version]: '$VER' != '$APP_V'"
  exit 1
fi
...
if [[ "$RID1" == "$RID2" ]]; then
  echo "FAIL [request_id uniqueness]: same ID on two calls - cached: $RID1"
  exit 1
fi
