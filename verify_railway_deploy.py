...
Usage: python verify_railway_deploy.py [railway_url] [vercel_url]
...
EXPECTED_SHA = "177fe815"              # control-plane-v1-clean latest build metadata commit
...
# URLs
RAILWAY_URL = sys.argv[1] if len(sys.argv) > 1 else "https://web-production-74ed0.up.railway.app"
VERCEL_URL = sys.argv[2] if len(sys.argv) > 2 else "https://torq-console.vercel.app"
...
def main() -> int:
    ...
    try:
        health = fetch_json(f"{RAILWAY_URL}/health")
    ...
    if git_sha:
        if EXPECTED_SHA in git_sha:
            ...
            warn_msg(f"Git SHA: {git_sha} (expected prefix: {EXPECTED_SHA})")
            exit_code = 2
    else:
    ...
    try:
        deploy = fetch_json(f"{RAILWAY_URL}/api/debug/deploy")
    ...
    running_file = get_nested(deploy, "running_file")
    if running_file and "railway_app" in running_file:
        pass_msg(f"Running correct module: {running_file}")
    else:
        warn_msg(f"Unexpected running_file: {running_file}")
    ...
    try:
        telemetry = fetch_json(f"{RAILWAY_URL}/api/telemetry/health")
    ...
    # ========================================================================
    header("Test 4: Vercel Proxy Relay (via Vercel to Railway)")

    try:
        vercel_health = fetch_json(f"{VERCEL_URL}/api/telemetry/health")
        print(json.dumps(vercel_health, indent=2)[:500] + "...")

        vercel_key_type = get_nested(vercel_health, "key_type_detected")
        vercel_project_ref = get_nested(vercel_health, "supabase_project_ref")

        if vercel_key_type:
            pass_msg(f"Vercel proxy relays key_type_detected: {vercel_key_type}")
        else:
            warn_msg("Vercel proxy not relaying telemetry health (rewrite config?)")

        if vercel_project_ref == EXPECTED_PROJECT_REF:
            pass_msg("Vercel proxy relays correct project ref")
        else:
            warn_msg(f"Vercel project ref: {vercel_project_ref}")
    except Exception as e:
        warn_msg(f"Vercel proxy unreachable (may not be configured yet): {e}")
        print("Skipping Vercel validation...")
    print()
    ...
    if exit_code == 0:
        ...
        print("  - Branch: control-plane-v1-clean")
        print(f"  - Commit: {deploy_git_sha}")
        ...
        print()
        print("Common issues:")
        print("  - Old code still running: Trigger new deployment in Railway")
