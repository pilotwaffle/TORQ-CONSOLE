#!/usr/bin/env python3
"""
Simple test script for TORQ Console with Perplexity search integration
Creates a newsletter about NTT's Expert Knowledge Visualization AI
"""
...
async def test_newsletter_creation():
    ...
    try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if 'response' in result:
                        ...
                        if word_count >= 1000:
                            print("SUCCESS: Newsletter meets minimum length requirement")
                        else:
                            print("WARNING: Newsletter is shorter than expected")
        ...
        print("Request timed out - the search and newsletter generation may take longer")
        return False
...
async def test_perplexity_direct():
    ...
    try:
        ...
        if result:
            print("SUCCESS: Direct Perplexity test successful")
        else:
            print("FAILED: Direct Perplexity test failed")
        return result
    except Exception as e:
        print(f"ERROR: Direct Perplexity test error: {e}")
        return False
...
async def main():
    ...
    if success1 and success2:
        ...
        print("Some tests failed - check the console logs for details")
