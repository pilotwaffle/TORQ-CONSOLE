...
'''
    ...
    def _generate_sql_template(self, description: str) -> str:
        """Generate SQL template with security best practices.
        
        Args:
            description: Description of the SQL query purpose
            
        Returns:
            SQL template with parameterized query examples and security notes
            
        Security Note:
            This template emphasizes parameterized queries to prevent SQL injection.
        """
        # Sanitize description to prevent injection in SQL comments
        safe_description = description.replace('*/', '* /').replace('--', '- -')[:200]
        
        return f'''-- {safe_description}
-- Generated by Prince Flowers AI
-- ⚠️  SECURITY: Always use parameterized queries, never string concatenation

-- ✅ GOOD: Parameterized query (prevents SQL injection)
/*
-- Python example with psycopg2
cursor.execute(
    "SELECT column1, column2 FROM table_name WHERE condition = %s ORDER BY column1",
    (user_input,)
)
...
-- TODO: Add your parameterized SQL queries here
'''
