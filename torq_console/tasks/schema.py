"""
Database schema for Autonomous Task Graph Engine.

Run this SQL in Supabase to create the tables.
"""

# SQL Schema for Task Graph Engine
TASK_GRAPHS_SCHEMA = """
-- ============================================================================
-- Task Graphs Table
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS task_nodes (
    ...
    -- Dependencies (comma-separated node_ids)
...
CREATE POLICY "task_graphs_write_service" ON task_graphs
    FOR ALL USING (auth.uid() = (current_setting('request.jwt.claim.sub')::UUID))
    WITH CHECK (true);

CREATE POLICY "task_nodes_read_all" ON task_nodes
    FOR SELECT USING (true);

CREATE POLICY "task_nodes_write_service" ON task_nodes
    FOR ALL USING (auth.uid() = (current_setting('request.jwt.claim.sub')::UUID))
    WITH CHECK (true);
...
# Migration function to run the schema
async def run_task_graph_migration(supabase_client):
    """Execute the task graph schema migration."""
    from .models import TaskGraph, TaskNode, TaskEdge, TaskExecution

    # Split and execute statements
    statements = TASK_GRAPHS_SCHEMA.split(';')
    for stmt in statements:
        stmt = stmt.strip()
        if stmt and not stmt.startswith('--'):
            try:
                result = supabase_client.table('_').rpc('exec_sql', {'sql': stmt})
                print(f"Executed: {stmt[:50]}...")
            except Exception as e:
                # Some statements might fail if they already exist
                print(f"Note: {stmt[:50]}... - {e}")
